...
Trailers.Showtimes.prototype = {
	...
	{
		ATVUtils.log( "PROCESSTEMPLATE: THEATRE DATA: ==>\n\n"+ JSON.stringify( this.theatreData ) +"\n\n<==", 4 );
		ATVUtils.log( "PROCESSTEMPLATE: MOVIE DATA: ==>\n\n"+ JSON.stringify( this.movieData ) +"\n\n<==", 4 );
	...
	sortCompleteShowtimeList: function() 
	{
		function sortByShowtimes( a, b ) {
			...
			atime += ( showA[1].toUpperCase() == "PM" && parseInt( showAtime[0] ) != 12 ) ? twelveHours : 0;
			btime += ( showB[1].toUpperCase() == "PM" && parseInt( showBtime[0] ) != 12 ) ? twelveHours : 0;

			results = atime - btime;

			ATVUtils.log( "a: "+ atime + " b: "+ btime +" results: "+ results, 3 );
